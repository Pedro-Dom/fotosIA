...
import {ReactComponent as IA} from '../src/images/ai.svg'
import './App.css'
import Carregando from '../src/images/Cube-1.6s-200px.gif'
...
function ListaPessoas(props){
  ...
  <img key={pessoa.id} src={pessoa.urls[4][512]}
       title="Pessoa gerada via IA" alt="pessoa gerada via inteligência artificial"/>
  )
  return(
    <>{listagemPessoas}</>
  )
 ...
 async function obtemFoto(){ // Quando for pegar dados externos!!! asyc e await (loading)
  setCarregando(true)
  ...
  return (
    <div className="app">
      ...
      <IA/>
      {carregando &&
      <img src={Carregando} tilte= "Aguarde..." alt="Aguarde" withd="75" />}
      <div className='linha'>
